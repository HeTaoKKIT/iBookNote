define(['../highlight/highlight', 'jquery'],
  function (highlight, $) {
    'use strict';
    // define GlobalConfig to set global static var
    var GlobalConfig = (function(){
      var letDefinedList = {
        'debug'                 :   'false',
        'isServer'              :   'true',
        'serverDomain'          :   'http://196.168.43.1:8888',
    ...
    // CommonUtil.globalVar = 'globalVar';
    /*
    CommonUtil.stringHandle = {
      formatHTML : function(mixHTML){
        //foo
      }
    };
    */
    ...
    CommonUtil.editPageElemetnContent = function(isEditAreaBigString, editIDName, savedIDName, hiddenStorageIDName, setCSSNotEditable) {
      ...
      document.getElementById(editIDName).onclick = function(){
        document.onclick = function(event) {
          ...
          // $(event.target).parent(); // get the jQuery element
        };
      }
      document.getElementById(savedIDName).onclick = function() {
        if(isEditArea) {
          document.getElementById(hiddenStorageIDName).insertAdjacentHTML('beforeend', '<div id="' + ( version + versionNumber ) + '"></div>');
            // document.getElementById(version + versionNumber).appendChild(document.querySelector('*[contenteditable]').cloneNode(true));
          document.getElementById(version + versionNumber).appendChild(document.body.cloneNode(true));
            // document.getElementById("SavedId").cloneNode(true) // copy the node full info with parameter true
            // for (var i = 0; i < document.querySelectorAll('*[contenteditable]').length; i++) { // querySelectorAll will return NodeList ObjectList, it's not a array type
          ...
          dataBase.tempData           =    CommonUtil.getCurrentDate().substr('4', '11');
          dataBase.VersionDateIndex   =    dataBase.tempData.replace(/ /g, '') + versionNumber.toString();
          dataBase.changeDate         =    dataBase.tempData;
          dataBase.changeFilePath     =    document.URL.split('iBookNoteShare')[1]; // document.baseURI IE doesn't support
    ...
    CommonUtil.dataBaseInit = function() {
      ...
      if(!iBNdb) { alert('您的浏览器不支持HTML5本地数据库'); return; }
    ...
    CommonUtil.dataBaseUpdate = function(data) {
      ...
      iBNdb.transaction(function (trans) {
          trans.executeSql(
            ...
            [data.VersionDateIndex, data.versionNumber, data.changeDate, data.changeFilePath, data.platform, data.modifyBody, data.submitComment], function (ts, data) {
          }, function (ts, message) {
              alert(message);
          });
      });
